...
// ========== counter ==========
const WAVE_STATS_PATH = path.join(__dirname, '../../../data/wave_stats.json');
...
const saveStats = (s) => { try { fs.writeFileSync(WAVE_STATS_PATH, JSON.stringify(s, null, 2), 'utf8'); } catch (e) { console.error('failed to save wave stats:', e); } };
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (targetUser && targetUser.id !== interaction.user.id) {
        ...
        desc = `-# **${interaction.user.username}** waves at **${targetUser.username}!** (●'◡'●)\n` +
          `-# ***${interaction.user.username}** has waved at **${targetUser.username}** ${count} ${count === 1 ? 'time' : 'times'}.*`;
      } else {
        desc = `-# **${interaction.user.username}** waves at literally no one. brave.`;
      }
      ...
      collector.on('collect', async i => {
        ...
        const responseEmbed = new EmbedBuilder()
          .setDescription(`-# **${targetUser.username}** chose to **${action}** back at **${interaction.user.username}!**`)
