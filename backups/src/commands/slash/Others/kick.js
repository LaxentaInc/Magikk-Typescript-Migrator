...
// ========== counter ==========
const STATS_PATH = path.join(__dirname, '../../../data/kick_stats.json');
...
const saveStats = (s) => { try { fs.writeFileSync(STATS_PATH, JSON.stringify(s, null, 2), 'utf8'); } catch (e) { console.error('failed to save kick stats:', e); } };
const incrementCount = (id1, id2) => { const s = loadStats(); const k = [id1, id2].sort().join(':'); s[k] = (s[k] || 0) + 1; saveStats(s); return s[k]; };

module.exports = {
    data: new SlashCommandBuilder()
        .setName('kickkkeee')
        .setDescription('NOT A MODERATION COMMAND, ACTION: Kick someone!')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (!gif) return interaction.editReply({ content: 'couldn\'t fetch a kick gif. try again!' });
            const count = incrementCount(interaction.user.id, target.id);
            const embed = new EmbedBuilder()
                .setDescription(`-# **${interaction.user.username}** kicks **${target.username}!**\n-# ***${interaction.user.username}** has kicked **${target.username}** ${count} ${count === 1 ? 'time' : 'times'}.*`)
                .setImage(gif);
            await interaction.editReply({ embeds: [embed] });
        } catch (e) { console.error(`error: ${e.message}`); await interaction.editReply({ content: 'failed to kick!' }); }
    }
};
